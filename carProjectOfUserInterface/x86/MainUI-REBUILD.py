...
class CarUI:
    ...
    def MainUI(self):
        ...
        self.Combox.place(x=self.ScreenWidthMiddle,
                          y=self.ScreenHeight//25, anchor="center")
        self.StartButton.place(x=self.ScreenWidthMiddle,
                               y=self.ScreenHeight//12, anchor="center")
        self.AutoButton.place(x=self.ScreenWidthMiddle,
                              y=self.ScreenHeight//6, anchor="center")
    ...
    def Dashboard(self):
        try:
            ...
            LeftButton = ttk.Button(self.root, text="←TurnLeft", width=self.ScreenWidth//140, style=INFO,
                                    command=self.SignalSystem.TurnLeftSignal).place(x=self.ScreenWidth//18, y=self.ScreenHeightMidddle, anchor="center")
            RightButton = ttk.Button(self.root, text="TurnRight→", width=self.ScreenWidth//140, style=INFO,
                                     command=self.SignalSystem.TurnRightSignal).place(x=self.ScreenWidth-self.ScreenWidth//18, y=self.ScreenHeightMidddle, anchor="center")
            ...
            EmergencyButtonGreen = ttk.Button(self.root, text="Turn On Emergency Light", width=self.ScreenWidth//110, style=FIRST,
                                              command=self.SignalSystem.AutoEmergencyLight)
            EmergencyButtonRed = ttk.Button(self.root, text="Turn Off Emergency Light", width=self.ScreenWidth//110, style=SECONDARY,
                                            command=self.SignalSystem.AutoEmergencyLight)
            ...
            EmergencyButtonGreen.place(x=self.ScreenWidth-self.ScreenWidth //
                                       18, y=self.ScreenHeight-self.ScreenHeight//2.7, anchor="center")
            EmergencyButtonRed.place_forget()
            ...
            while True:
                ...
                self.SpeedLeft = self.BackgroundCanvas.create_text(self.ScreenWidth//7, self.ScreenHeight//5, text=ReadList[
                    "SLeft"], font=("", self.ScreenHeight//8), fill="#EDA69F")
                self.SpeedRight = self.BackgroundCanvas.create_text(self.ScreenWidth-self.ScreenWidth//5, self.ScreenHeight//5, text=ReadList[
                    "SRight"], font=("", self.ScreenHeight//8), fill="#EDA69F")
                print(ReadList)

                if time.time()-self.SignalSystem.CommandSendTime > 1.5:
                ...
                print(ReadList["CmdCode"], "hello?", ForeCommandCode)
                if ReadList["CmdCode"] == "01" and not EmergencyButtonRed.winfo_ismapped() and EmergencyButtonGreen.winfo_ismapped():
                    EmergencyButtonGreen.place_forget()
                    EmergencyButtonRed.place(x=self.ScreenWidth-self.ScreenWidth//18,
                                             y=self.ScreenHeight-self.ScreenHeight//2.7, anchor="center")
                elif ReadList["CmdCode"] == "-1" and EmergencyButtonRed.winfo_ismapped() and not EmergencyButtonGreen.winfo_ismapped():
                    EmergencyButtonRed.place_forget()
                    EmergencyButtonGreen.place(
                        x=self.ScreenWidth-self.ScreenWidth//18, y=self.ScreenHeight-self.ScreenHeight//2.7, anchor="center")
                ForeCommandCode = ReadList["CmdCode"]
    ...
    def AutoProcess(self):
        result = AutoCheck(self.OS).result
        if result == None:
            Thread(target=self.MessageBoxThread, args=(
                "Auto Detect Error", "未找到指定设备，请检查设备是否正常连接或正确刷入程序")).start()
            self.Combox.place(x=self.ScreenWidthMiddle,
                              y=self.ScreenHeight//25, anchor="center")
            self.StartButton.place(
                x=self.ScreenWidthMiddle, y=self.ScreenHeight//12, anchor="center")
            self.AutoButton.place(x=self.ScreenWidthMiddle,
                                  y=self.ScreenHeight//6, anchor="center")
        else:
    ...
    def AutoText(self):
        ...
        def Text1():
            text = self.BackgroundCanvas.create_text(self.ScreenWidth//2, self.ScreenHeight // 3, text="正在检测中", font=(
                "", self.ScreenHeight//25), fill="#4A4A4A", anchor="center")
            for i in animationObj("#4A4A4A", "#FFFFFF").gradient():
                self.BackgroundCanvas.delete(text)
                text = self.BackgroundCanvas.create_text(
                    self.ScreenWidth//2, self.ScreenHeight // 3, text="正在检测中", font=("", self.ScreenHeight//25), fill=i, anchor="center")
                self.BackgroundCanvas.update()
                time.sleep(0.005)
            time.sleep(1.5)
        ...
        def Text2():
            text = self.BackgroundCanvas.create_text(self.ScreenWidth//2, self.ScreenHeight // 3, text="很快就好", font=(
                "", self.ScreenHeight//25), fill="#4A4A4A", anchor="center")
            for i in animationObj("#4A4A4A", "#FFFFFF").gradient():
                self.BackgroundCanvas.delete(text)
                text = self.BackgroundCanvas.create_text(
                    self.ScreenWidth//2, self.ScreenHeight // 3, text="很快就好", font=("", self.ScreenHeight//25), fill=i, anchor="center")
                self.BackgroundCanvas.update()
                time.sleep(0.005)
            time.sleep(1.5)
            for i in animationObj("#FFFFFF", "#4A4A4A").gradient():
